...
In the example above, vue adds the class `active` if isActive is true. Otherwise, it adds the class `text-danger` to the element.

❗ Note: You can have both the static as well as the dynamic attribute bind to the element (class & :class).
